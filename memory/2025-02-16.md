...
## OpenClaw Security Audit & Hardening 🔐

José asked me to do a "secret mission" — study how OpenClaw works and identify attack vectors and dangerous permissions. Then implement least-privilege access controls.
...
1. **No sandboxing enabled** — all exec runs directly on the host
2. **No exec-approvals.json configured** — no allowlist or user approval prompts
...
- main (me) — full owner access
- pygmoo-cto — tech lead needs it
- pygmoo-fullstack — dev needs it
- kai — mentor for code review
...
### 🐛 Discovery: Sandbox Workspace Isolation
...
- Creator can't see either — only `/workspace-pygmoo-curriculum-creator/`
...
### PygMoo Course 2: Barbería 101 🐷💈

CPO decided Course 2 = Barbería (not Fonda/Food Truck):
- Different sector from Taquería (Course 1)
- Low startup cost
...
- ❌ Can't access other agent workspaces
- ❌ Can't spawn other agents (sessions_spawn blocked)
- ❌ Can't message other agents (sessions_send blocked)
- ✅ All orchestration flows through main agent

This is the intended security model — main is the chokepoint.

---

## Final Decision: Simplify 🧹

After testing sandboxing extensively, José made the call: **deactivate all PygMoo agents**.
...
**Final state:**
- 22 agents → 2 agents (main + astrid-ep)
...
**Docker gateway option:** Prepared Dockerfile + docker-compose at `~/.openclaw/docker/` for running entire OpenClaw in container. Not deployed yet — requires manual switchover.
